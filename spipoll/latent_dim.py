...
row_index = 0
for D_plus in range(10):
    for D_minus in range(10):
        if D_plus+D_minus>0 :
            args.hidden2_dim1 = D_plus + D_minus
            args.hidden2_dim2 = args.hidden2_dim1
            ...
            for k in range(10):
                ...
                model = VBGAE3(adj_norm,species_index,D_minus) 
                init_parameters(model)
                optimizer = Adam(model.parameters(), lr=args.learning_rate)
                # train model
                ...
                model2 = VBGAE3(adj_norm,species_index,D_minus)
...
result = pandas.read_csv("spipoll_latent_dim.csv")

res_matrix = np.zeros((10,10))
for D_plus in range(10):
    for D_minus in range(10):
        if D_plus+D_minus>0 :
            res_matrix[D_plus,D_minus]= result[(result["D_plus"]==D_plus)&(result["D_minus"]==D_minus)]["AUC"].mean()
            
    
res_matrix[0,0]=0.5
plt.imshow(res_matrix,vmin=result["AUC3"].min(), origin ="lower")
plt.xlabel(r"$D_-$")
plt.ylabel(r"$D_+$")
for (j,i),label in np.ndenumerate(res_matrix):
    plt.text(i,j,int(np.round(label,2)*100),ha='center',va='center')
plt.title(r"$AUC(\widehat{B})$ BVGAE")
plt.show()
...
for D_plus in range(10):
    for D_minus in range(10):
        if D_plus+D_minus>0 :
            res_matrix[D_plus,D_minus]= result[(result["D_plus"]==D_plus)&(result["D_minus"]==D_minus)]["AUC3"].mean()
...
result = pandas.read_csv("spipoll_fair_latent_dim.csv")
...
for (j,i),label in np.ndenumerate(res_matrix):
    plt.text(i,j,int(np.round(label,2)*100),ha='center',va='center')
plt.title(r"$AUC(\widehat{B})$ fair-BVGAE")
plt.show()
