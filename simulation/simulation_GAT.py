...
for epoch in pbar:
    ...
    Z = model(torch.Tensor(features01),adj_train2)
    A_pred=distance_decode(Z,Z)
...
plt.scatter(Z.detach().numpy()[:,0],Z.detach().numpy()[:,1],c=z1[:,0])
...
features1 = torch.Tensor(features01) 
features2 = torch.Tensor(features02) 


torch.manual_seed(2)
model = BGAT(1000,100,2,2,0.2,10)
adj_train, train_edges, val_edges, val_edges_false, test_edges, test_edges_false = mask_test_edges(adj0)
adj_train2=torch.Tensor(adj_train.todense())
...
fig = plt.figure()
ax = fig.add_subplot()
plt.scatter(Z1[:,0].detach().numpy(),
            Z1[:,1].detach().numpy(),
            s=10,
            label=r"$Z_1$",
            c=s)
plt.scatter(Z2[:,0].detach().numpy(),
            Z2[:,1].detach().numpy(),
            s=20,label=r"$Z_2$",
            marker="^",c="r")
ax.legend()
...
torch.corrcoef(torch.cat([Z1.detach(),torch.Tensor(z1)],axis=1).T)
S = torch.Tensor(s)

stat2 = HSIC_stat(Z1.detach(),S)
n=1000
x = np.linspace(0, 0.4, 100)
y = stats.gamma.cdf(x,stat2[3].item(),scale=stat2[4].item())
plt.plot(x,y)
...
torch.manual_seed(2)
model2 = BGAT(1000,100,2,2,0.2,10)
...
for epoch in pbar:
    ...
    optimizer.zero_grad()
    loss = norm*F.binary_cross_entropy(A_pred.view(-1), adj_label.to_dense().view(-1), weight = weight_tensor)
    independance =n1*(RFF_HSIC(Z1,S))#0.02
...
result_BGAT = pandas.DataFrame(columns = ["AUC","AP","HSIC","p_value","#0.05","cov_1"],
                          index = range(100))



fair_result_BGAT = pandas.DataFrame(columns = ["AUC","AP","HSIC","p_value","#0.05","cov_1"],
                          index = range(100))
...
for k in range(100):
    ...
    test_roc, test_ap = get_scores(test_edges, test_edges_false, A_pred)
    stat2 = HSIC_stat(Z1,S)
    zut=stats.gamma.sf(stat2[0].item()*n1, stat2[3].item(), scale=stat2[4].item())
    cov2 = torch.linalg.norm(torch.corrcoef(torch.cat([Z1,torch.Tensor(z1)],axis=1).T)[2,:2]).item()

    result_BGAT.iloc[k] = [test_roc,test_ap,stat2[0].detach().numpy() ,zut,zut<0.05,cov2]
    print(result_BGAT.iloc[k])
    ...
    fair_result_BGAT.iloc[k] = [test_roc,test_ap,stat2[0].detach().numpy() ,zut,zut<0.05,cov2]
    print(fair_result_BGAT.iloc[k])
    result_BGAT.to_csv("simulation_results/result_BGAT.csv")
    fair_result_BGAT.to_csv("simulation_results/fair_result_BGAT.csv")
