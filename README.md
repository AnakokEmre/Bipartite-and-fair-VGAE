...
First, we see that $||S^\top Xv||^2 = 0 \implies S^\top Xv = 0$, which allows us to plug into the derivative equation:
...
For multiple random realisations of $X$, we compute $K_{i,j} = e^{-\frac{||x_i-x_j||^2}{2}}$ and then plug it in the equation. We also compute the gradient with respect to $X$ using Pytorch automatic differentiation, before measuring the average time taken to realize both of these operations.
